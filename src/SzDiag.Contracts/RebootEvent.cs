...
/// ~6 часами тремя сеансами со штатными выключениями между ними). Null для
/// <see cref="ShutdownKind.Sleep"/> — там вместо этого заполнен <c>DurationSeconds</c>.</param>
...
public sealed record PowerEvent(DateTimeOffset At, string Kind, long Bugcheck = 0,
    long? UptimeBeforeSeconds = null, long? DurationSeconds = null)
{
    public TimeSpan? UptimeBefore =>
        UptimeBeforeSeconds is { } s ? TimeSpan.FromSeconds(s) : null;
}
