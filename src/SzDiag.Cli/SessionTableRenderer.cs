...
public static class SessionTableRenderer
{
    ...
    /// порога — «лаг?», после — «нет связи» (нейтрально, БЕЗ уверенного «ВЫРУБОН»: смена
    ...
    /// только по счётчику ⚡N и подсветке uptime, а не в самом статусе (бэклог п.42).
    ///
    /// Уверенного «ВЫРУБОН» здесь сознательно нет (Important-1, ревью волны 1): единственное
    /// достоверное подтверждение реального отказа — смена boot-time при реконнекте
    /// (<see cref="SessionInfo.LastRebootAt"/>/<see cref="SessionInfo.RebootCount"/>), а её,
    /// пока СЗ ещё offline, попросту неоткуда взять — она появляется только вместе с
    /// возвращением в online (<see cref="UptimeCell"/> подсвечивает свежий ребут и счётчик
    /// там). Само по себе долгое молчание — не вырубон: «под многочасовым OCCT 10 минут
