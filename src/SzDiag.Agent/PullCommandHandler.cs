...
public sealed class PullCommandHandler
{
    ...
    /// <summary>Ошибка при sharing violation дополняется тем, КТО держит файл: молчаливое
    /// "занято чем-то" бесполезно, когда нужно решить, ждать закрытия сессии или нет (бэклог
    /// п.104) — если писатель (например живой `sshd`) открыл файл вовсе без совместного
    /// чтения, прочитать байты без VSS всё равно нельзя, но назвать держателя можно.</summary>
