...
- **Агент переживает ребут + авто-реконнект под тем же СЗ** — боль дня 2026-07-23: под
  нагрузочными тестами клиент ребутался по 5-10 раз, и **каждый раз агента поднимали вручную**,
  а СЗ уходила offline, SSH отваливался (фикс «добить зависший sshd при старте» — только полумера,
  агента всё равно надо запустить). Идея: агент помнит свой номер СЗ (persist в файл рядом с exe),
  стартует как служба/через автозапуск и сам переподключается к hub после ребута под тем же СЗ.
  Убирает половину ручных «подними агента» при стресс-диагностике. Спеки нет.
