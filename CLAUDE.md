...
- **Наполнение базы знаний** паттернами «симптом → причина» по мере заявок. Паттерн —
  ...
  - **Системный вывод:** заводской XMP 6000 на Zen4 без «X» (7500F) — нестабилен на разных
    платах (ASUS B650, MSI B850) → **занижать заводской профиль до 5600** на таких сборках.
...
- **SMB-транспорт доставки файлов агенту** — боль: ssh-stdin через портативный sshd под
...
- **RunDiag: bugcheck-коды сразу hex + имя** (quick win) — `diag.md` выдаёт `BugcheckCode=190`
  в decimal, каждый кейс приходится вручную переводить в `0xBE ATTEMPTED_WRITE_TO_READONLY_MEMORY`.
  Пусть секция `reboots`/BugCheck сразу пишет hex + название стоп-кода (таблица 0x1A/0x4E/0x50/
  0xBE/0x101/0x124/0x13A…). Дёшево, экономит шаг на каждом разборе BSOD.
