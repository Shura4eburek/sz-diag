# Параметры правь здесь. param() НЕ использовать: агент подставляет перед скриптом свою
# строку ($ProgressPreference=...), поэтому param оказывается не первой инструкцией и
# PowerShell роняет скрипт на 'The assignment expression is not valid' (СЗ 161498).
