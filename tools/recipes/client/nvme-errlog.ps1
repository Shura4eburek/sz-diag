...
#
# ВНИМАНИЕ (бэклог п.133/70, СЗ 161346): карта ниже строится на СЕЙЧАС (Get-PhysicalDisk/
# Win32_DiskDrive) — нумерация PhysicalDrive/HarddiskN/RaidPortN плывёт от загрузки к
# загрузке (порядок подключения). Для архивных ошибок (журнал за прошлые дни/недели) эта
# карта может дать ЗЕРКАЛЬНУЮ привязку, если диски успели поменяться местами — используй
# tools\recipes\client\disk-number-history.ps1, который резолвит номер на момент события
# через Partition/Diagnostic 1006.
