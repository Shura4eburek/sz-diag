...
#
# Грабля (161716, бэклог п.184): y-cruncher буферизует вывод и дописывает лог редко — рецепт
# читал «последняя запись давно» как «машина вырубилась», хотя процесс был жив (Kizuna.exe).
# «ОБОРВАН» теперь только когда процесс МЁРТВ И лог не растёт; живой процесс = буферизация,
# не вырубон, каким бы старым ни был `LastWriteTime`.
$done   = $txt | Select-String -Pattern 'Test Finished' -Quiet
$passed = ($txt | Select-String -Pattern ':\s*Passed').Count
$verdict = if ($done) {
    'прогон ДОШЁЛ до конца'
} elseif ($kid) {
    $lagMin = [math]::Round(((Get-Date) - $f.LastWriteTime).TotalMinutes, 1)
    "лог отстаёт на $lagMin мин (буферизация) — тест идёт, процесс жив"
} else {
    '⚠ лог ОБОРВАН — смотри szcli reboots, машина могла вырубиться'
}
"итог: {0}, проверок Passed: {1}" -f $verdict, $passed
