...
if (-not $g -and $bots -gt 0) {
    # I-14 (ревью волны 2): матч ШЁЛ (боты в свежем console.log есть), а потом cs2.exe пропал —
    # для заявки «вылетает в игре» это и есть НАЙДЕННЫЙ ОТКАЗ, а не «прогон не смог стартовать».
    # Раньше эта же ветка (bots>0, процесса нет) подписывалась «cs2.exe НЕ ПОДНЯЛСЯ» и отправляла
    # техника чинить Steam/StateFlags вместо того, чтобы снимать сенсоры/дампы с воспроизведённого
    # дефекта.
    throw "ОТКАЗ ВОСПРОИЗВЕДЁН: карта была под нагрузкой (ботов в console.log: $bots), а cs2.exe ПРОПАЛ во время матча — это и есть находка по заявке, не сбой прогона. Снимай сенсоры/минидампы, Steam чинить не нужно."
} elseif (-not $g) {
    throw "ОШИБКА: cs2.exe НЕ ПОДНЯЛСЯ — прогон НЕ СОСТОЯЛСЯ (console.log не создан вовсе или в нём нет ботов). Проверь Steam/StateFlags и запусти заново."
} elseif ($bots -eq 0) {
    "ОШИБКА: cs2.exe жив (pid $($g.Id)), НО КАРТА НЕ ЗАГРУЗИЛАСЬ — ботов в console.log: 0, игра стоит в меню. Прогон НЕ засчитывать."
} else {
    # I-15 (ревью волны 2): критерий #133 — нельзя рапортовать успехом, если GPU >= 60 % не
    # набралось за окно приёмки. Число ботов подтверждает только запуск матча, а не то, что
    # железо реально грузится (в меню CS2 тоже даёт ~30 % GPU) — финальное слово за приёмкой
    # нагрузки, не за этой строкой.
    ('запуск подтверждён: cs2.exe жив, pid ' + $g.Id + ', RAM ' + [int]($g.WorkingSet64/1MB) + ' МБ; ботов в матче: ' + $bots +
     '. Это НЕ «ПРОГОН ЗАСЧИТАН» — тот вердикт даёт только game-load-check.ps1 по факту GPU >= 60 %.')
}
'приёмка нагрузки (обязательна для "ПРОГОН ЗАСЧИТАН") — game-load-check.ps1 (GPU >= 60 % большую часть замеров)'
