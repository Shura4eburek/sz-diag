...
$Drives      = 'C:'          # ← куда пишем. На 161346 цель — системный том ТЕСТОВОГО диска сервиса
#            (18.08: SNV2S250G, он же C:, перенесён в слот CPU — контроль к прогону 17.08 в чипсетном)
# 18.08: окно короче вчерашнего — помещение обесточивают после 19:00, а старт вышел в 16:52
# (не 16:15: время на клиенте сверил только по факту запуска вахты). Прогон обязан закончиться
# сам, иначе его оборвёт рубильник и финальный SMART снять будет нечем — то есть весь смысл
# контрольного прогона потеряется. Перезапуск после холостого старта вышел в ~17:00,
# отсюда 105 мин: финиш ~18:45, запас на снятие SMART до обесточивания.
$Minutes     = 105
$FileGB      = 4             # размер рабочего файла на диск (17.08: 4 ГБ × 55 проходов = 220 ГБ —
#            держим тот же профиль, иначе прогоны в двух слотах несравнимы)
$BlockMB     = 4             # блок записи: крупный, чтобы гнать очередь, а не IOPS
# Грабля (161346, 18.08): порог 40 ГБ рассчитан на клиентский диск, а на ТЕСТОВОМ диске сервиса
# (233 ГБ, забит стендовым софтом — бенчмарки, игры) свободно всего 19.6 ГБ. Прогон 17.08 шёл
# со сниженным порогом, но правка осталась только на клиенте и в репо не вернулась — 18.08
# запуск ушёл в холостой цикл пропусков и 10 минут теста потеряны. Порог правим здесь, а не на месте.
$MinFreeGB   = 10            # ниже этого порога свободного места не опускаемся ни при каких
...
$WriteCapGB  = 400           # 17.08 прогон уложился в 220 ГБ из этого бюджета — держим тот же
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        foreach ($drv in $Drives) {
            ...
            if ($freeGB - $FileGB -lt $MinFreeGB) {
                # Пропуск НЕ должен крутиться вхолостую: без паузы цикл молотит тысячи итераций
                # в секунду и раздувает лог (18.08: 930 КБ за минуту, и это единственное, что
                # было в выводе задачи — сам факт «тест не идёт» тонул в мусоре).
                Say "  $drv — свободно $freeGB ГБ, файл $FileGB ГБ упрётся в порог $MinFreeGB ГБ. ПРОПУСК"
                Start-Sleep -Seconds ([Math]::Max(30, $PauseSec))
                continue
            }
