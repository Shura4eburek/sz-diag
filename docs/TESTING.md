...
> **Куда класть на клиенте: `C:\szdiag`, НЕ на рабочий стол.** Рабочий стол в Windows 11
> по умолчанию синхронизируется в OneDrive, а из облачной папки апдейтер работать
> отказывается (код 4, `CloudInstallGuard`, бэклог п.41) — логи и `state.json` сессии уехали
> бы в личное облако клиента. На 161642 это стоило дня: апдейтер «показывал окно и сразу
> закрывался», хотя причину честно печатал — прочитать её было нельзя, окно закрывалось
> вместе с процессом. Теперь при ненулевом коде окно ждёт Enter (`ConsolePause`), а вывод
> дублируется в `logs\updater.log`. Если и лога нет — запусти `updater-log.cmd` (от админа):
> он перехватит вывод в `updater-run.txt` и покажет дату/размер exe.
...
- **…а может и умереть совсем.** На 161312 после FurMark процесса `sshd` на
  машине не осталось вовсе (`sshd=0`), и сам он не вернулся — канал пропал на
  час, пока hub показывал СЗ `online` (heartbeat идёт своим путём). Поэтому
  проверять надо **процесс**, а не только порт: `Get-Process sshd`. Сейчас за
  этим следит сам агент (`SshdWatchdog`, раз в минуту поднимает упавший sshd и
  пишет об этом в `agent.log`) — если канал не вернулся за пару минут, смотри
  туда, а не гадай.
...
и ресайз проверяются **только руками** — из перенаправленного вывода липкий режим не
включается по определению.
...
3. **Ресайз:** потянуть окно по ширине и высоте — панель перестраивается, логи
   продолжают скроллиться только под линией. Ужать окно ниже 10 строк — липкий режим
   выключается насовсем, дальше обычный вывод.
...
5. **Агент под нагрузкой:** запустить OCCT через `szcli test run` — панель обновляется,
   watchdog тикает вниз, `[C]`/`[Q]` работают (`Console.ReadKey` с регионом не конфликтует).
6. **Headless:** `agent.exe --resume <state.json>` из scheduled task — панели нет,
   `agent.log` пишется как раньше.
7. **WinPE:** `agent.exe --pe` — панель есть, в поле watchdog прочерк (в PE задача не
   ставится), режим подписан как `WinPE`.
8. **Старый conhost:** запустить hub в `conhost.exe` (не Windows Terminal) — либо панель,
   либо чистый линейный вывод, но **не** мусор из escape-кодов.
9. **Рубильник:** `"StickyHeader": false` в секции `Hub` файла `appsettings.json` —
   возвращается прежний разовый баннер. У агента — тот же ключ в его `appsettings.json`
   или `SZAGENT_StickyHeader=false`.

## Автотесты (без хоста/клиента)
```powershell
dotnet test    # вся логика, ~286 тестов
```
