...
- [ ] Desk из `dist\host\desk` двойным кликом: на СЗ без сессии — кнопки `claude` и `claude2`; сессия в каждом профиле отвечает (нет «не залогинен»), в шапке чата виден профиль.
- [ ] Автомод: обычная диагностика (`szcli list/exec/diag`) идёт без карточек разрешений; карточка появляется только на то, что классификатор не пропустил.
- [ ] Рестарт hub посреди хода → сессия продолжает работать, в «АРХИВ» не уходит (архив — только если СЗ нет дольше 3 минут).
